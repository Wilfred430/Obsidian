...
### Pop 的 M1：一個沒有這個天花板的建構式自回歸模型

`temp` 分支同時有一個新模型 **M1**（`ml/m1_{common,model,dataset,train,infer}.py`，2026-07-07
首版）：跟我們的生成式模型概念很像（自回歸逐塊生成、純監督模仿、cross-entropy 避免
mode-averaging），**但預測的是 32×32 自由格點座標，不是樹結構**——這正是繞開 B\*-tree
密度天花板的關鍵設計（M3 探針的教訓直接內建進 M1）。目前極早期（只冒煙測試 60 case，
`near` 指標僅 0.02，近乎隨機），下一步是規模化訓練（GPU、10 萬 case 級）——這正好是我們
這個 session 已經解決過的問題（1M 檔案讀取、GPU 訓練迴圈、e^(n/12) 對應的 size-power
加權、checkpoint 管理），是目前評估中、風險最低的貢獻切入點。

### 修正後的下一步

1. ~~繼續生成式 B\*-tree 的 post-hoc/RL 優化~~ → **不建議**，表示法天花板已證實，投報比低。
2. **評估協助 M1 規模化訓練**——用我們已驗證的訓練基礎設施（資料管線、GPU 迴圈、
   `case_report.py`/`diag_vrel.py` 的診斷方法論）加速 M1 從冒煙測試到可用權重。
3. Approach A vs B vs C：**C（電靜力法 + S1 壓縮）目前確定領先**（2.72 vs 生成式 3.32），
   若 M1 訓練成功可能再往下探。最終送出應以 C 為主力，B 保留作為對照/備援。
4. 這是團隊分工層級的決策（是否同步 pop 的 `temp` 分支、如何分工 M1 訓練），已回報給使用者。
