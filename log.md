...
## [2026-08-11] Correction | 逐條核對官方 `official guidelines/C_20260522 (3).pdf`，修正多處跟規格書不符的內容
- **Context**: 使用者提供官方競賽規格 PDF（`official guidelines/C_20260522 (3).pdf`，13 頁，含 Objective Function、Inputs、Hard/Soft Constraints 完整定義），要求逐條核對整個 vault 的內容並保證正確性。
- **發現一（最嚴重）**：**Total Score 加權公式在多篇筆記裡寫成 $e^{n_i}$，遺漏了 $/12$**，正確是 $e^{n_i/12}$（官方 PDF 明確寫 `e^{n_i/12}`）。這個錯誤 2026-07-01 已在 [[ICCAD_code/8_Winning_Strategy_and_Roadmap|第 8 篇]]頂部與 [[Fundamentals/ICCAD-Glossary|詞彙表]]訂正過，但當時**沒有回頭修** [[ICCAD/Problem/FloorSet-Detailed|FloorSet 規格詳解]]（唯一直接引用 PDF 公式的規格筆記本身）、[[ICCAD_code/3_Cost_Function_and_Penalty|3. Cost Function]]（標題還寫「e^n 總分加權」）、[[ICCAD_code/8_Winning_Strategy_and_Roadmap|第 8 篇]]「診斷一」內文本身（只有檔案最上方有訂正提示，內文原句沒改）、[[ICCAD_code/6_ML_Generative_BTree|第 6 篇]]，這次全部改成 $e^{n/12}$，並把錯誤倍率 $e^{99}\approx8\times10^{42}$ 訂正為 $e^{8.25}\approx3820$。
- **發現二**：**官方規格根本沒有「預先給定的固定畫布 $(W,H)$」這個輸入，也沒有「超出畫布判 $M=10$」這條硬約束**——PDF 的 Inputs 只列 blocks/terminals/連線矩陣，Hard Constraints 只有 4 類（Overlap-free、Area Targets and Dimensionality、Fixed-shape Immutability、Preplaced Immutability），畫布大小是解出來的 bounding box，用 `Area_gap` 扣分而非判死。[[ICCAD/Problem/FloorSet-Detailed|規格詳解]]原本寫「輸入：固定畫布尺寸(W,H)」+「Fixed-outline 硬約束：所有組件必須嚴格限制在(W,H)邊界內」，兩處都不成立，已改寫；[[ICCAD_code/3_Cost_Function_and_Penalty|3. Cost Function]]的硬約束清單原本漏掉 Fixed-shape/Preplaced 兩項也一併補齊；[[Fundamentals/VLSI-Floorplanning-101|VLSI 入門]]的硬規矩清單原本多列了「不超出畫布」一項，已移除並加註警語；[[ICCAD/Algorithms/Fixed-Outline-Floorplanning|Fixed-Outline 理論筆記]]、[[ICCAD/Floorplanning/Outline-Characteristics|Outline 特性筆記]]兩篇一般 VLSI 理論背景筆記加上警告區塊，說明這是產業界一般分類、不是本競賽的實際規則。
- **發現三**：[[ICCAD_code/3_Cost_Function_and_Penalty|3. Cost Function]]的 `Area_bbox` 公式漏了 $\min_i(x_i)/\min_i(y_i)$ 項（只在假設所有座標已貼齊原點時才成立），HPWL 公式用的是傳統 net-bounding-box 模型，跟官方 Eq.3 的「逐對模組中心點加權曼哈頓距離」是不同公式——都已訂正/加註清楚哪個是官方公式。
- **發現四**：[[ICCAD/Problem/FloorSet-Detailed|規格詳解]]的得分預估表「100% 軟約束違規=9.09」漏了「還要 2 倍慢」這個前提（純違規只到 7.39），已補上並新增官方原表另一列 50% 違規=2.72；Dataset 小節補上 Validation/Test 各 100 筆的數字（原本只寫了 Training）。
- **未動的部分**：`log.md` 歷史條目本身（含這篇之前提到 $e^n$/固定畫布的舊記錄）**刻意不改**——這是時間軸記錄「當時寫了什麼」，不是現在的事實陳述，已有訂正的地方會在對應日期後續條目或筆記本身裡體現。
- **驗證方式**：13 頁 PDF 全文逐頁讀過（Organizers/Background/Motivation/Related Work/Proof of Concept/Problem Statement/Objective Function/Interpretation/HPWL/Area/Violation Cost/Dataset/Total Score/Incentivizing ML/References），每一條修正都能對應到 PDF 裡的具體段落或公式，不是憑印象改。

## [2026-08-15] Update | MIB campaign：撿回一個沒跑完的實驗 + 把變換改成「可拒絕的候選」+ 同步官方 v10

- **Context**: 使用者 `/goal` 設定持續自動化優化直到 token 耗盡。這一輪的起點是「參數空間已經掃到見底」——約 46 個維度只有 2 個正向。於是改問一個不同的問題：**倉庫裡有沒有「寫好了、有守衛、但主實驗從沒跑完」的機制？**
- **關鍵發現一(方法論)**：`docs/superpowers/plans/2026-07-31-mib-unify-free-quality.md` 是一份完整的 5-Task 計畫，Task 2（加旗標）做完了、程式碼還在，但 **Task 3「全 100 案主實驗」從沒跑完**——核取方塊全未勾，`docs/` 底下也沒有任何戰報記錄過結果。它預設關閉不是因為輸，是因為**實驗被放棄**。撿回來實測是正的。`- [ ]` 沒勾完的 plan 文件是一份現成的候選清單，比從零想新機制便宜、比再掃參數命中率高。
- **關鍵發現二(機制)**：`MIB_UNIFY` **必須跟 `MIB_POST_SNAP` 成對開**——單獨開是拿 V_boundary 換 V_mib，配對才淨賺。**單旗標掃描永遠找不到這個組合。** 進一步發現無條件套用的代價集中在權重最高的 case 99（V_rel 0.0746→0.1045，而 hpwl 幾乎沒動），因為候選池裡從來沒有「未統一」的版本可選。`MIB_BOTH` 讓 `_finish` 把套用 MIB 之前的幾何也交出來，兩份都進池讓 proxy 逐案挑——case 99 **分毫不差地回到起點值**，毛惡化減半。**這修正了對「事後搬動一律失敗」的理解：真正的規則不是「不能有事後階段」，而是「不能無條件套用一個只盯單一目標的變換」；做成可拒絕的候選就是安全的，因為最壞情況等於不做。**
- **關鍵發現三(否決)**：Ji et al. (COR 2017) 的定理數學上完全正確（軟方塊 λ=∞ ⟹ 任兩個永遠能零死區合併），據此實作的「逐子樹 util」**全滅**——收走 100%/75%/50%/25% 的軟方塊餘裕，Feasible 各為 18/17/16/17（滿分 21）。**定理沒錯，套用對象錯了**：留白在這條管線裡不是堆疊死區，是修復階段的活動空間。另外「mib_unify 後補 grouping_repair」與「`DUAL_ASCENT_GRP` + MIB 配對」也都是負面，後者顯示**耦合會消耗掉下游機制的作用條件**（MIB 的修復力被 GRP 摧毀 82%），不只是分數取捨——「共用預算耦合」累計第六次確認。
- **關鍵發現四(量測紀律)**：REAL 的數字被修正了**三次**（−2.6% → −1.15% → −0.27%），而 Neutral 一次都沒錯。三次錯誤全在帶 RT 的指標，**而且每次都往樂觀方向錯**——這是結構性的，因為 RT 的每個雜訊源（`time.time()` 倒退、負 runtime 被夾到速度下限）都偏向讓分數變好。修法是把本地評測器的計時改成 `time.monotonic()`（壞掉的案例從每 100 案 2–4 個變成 0）。另外也發現**篩選腳本的 `2>/dev/null` 把「機制靜默關閉」的警告通道整個丟掉**，已加起飛前檢查。
- **關鍵發現五(官方同步)**：FloorSet checkout 落後上游 4 個 commit，其中 `fc28d6d "may 22 contest fixes"` 動了評測器。使用者先前手動照 PDF 改的三項（`exp(n)`→`exp(n/12)` 權重、feasible cost 上限 M−10⁻⁶、local RuntimeFactor=1.0 中性化）**一字不差就是官方後來在 v10 正式發布的修正**——等於提早一個月改對了。已同步到 `aadddcc`／規格 **v10**，只保留兩項刻意的本地差異（單調時鐘、`--test-id` 多值）。v10 也首次給出分數尺標：完美品質+中位數速度=1.00、3× 以上快（封頂）=**0.70 是理論下限**，我們的速度乘數已經是 0.7165，**速度只剩約 2.3% 可拿、品質還有 20.3%**。
- **成績**: 全 100 案 Neutral **1.2323 → 1.2252**（−0.58%），Feasible 全程 **100/100**。四項預設改動：`ELECTRO_MIB_UNIFY` 0→1、`ELECTRO_MIB_POST_SNAP` 0→1、`ELECTRO_MIB_PORTFOLIO` 1→0、`ELECTRO_MIB_BOTH` 0→1（新增）。
- **Update**: [[ICCAD_code/Electronic_Pipeline|Pipeline 說明]]——頂部改標 v22 + MIB campaign、成績表換成 v22 數字並改為「Neutral 確定性／REAL 給範圍／Avg Runtime 不當屬性報告」、Stage 3 小節新增三項 MIB 機制的完整說明（含成對開的 danger 與可拒絕候選的 success callout）、旗標清單新增 4 項、新增「研究方向紀錄（2026-08-15 MIB campaign）」表格（3 個轉正 + 5 個否決）。[[Electronic_Pipeline.canvas|畫布]]局部增修三個節點（node-001 標題與成績、node-012 Stage 3 收下 MIB 配對、node-032 角色不變但機制換成 MIB_BOTH 並標註 MIB_PORTFOLIO 已關閉）+ 新增一條邊 edge-142（node-032 → node-040，兩份候選確實會進 proxy 排名）；**其餘 19 個節點的座標與尺寸完全未動**，遵守「手動修正過的版面是基準」的鐵則。
- **Note**: 另外產出兩份給使用者自學用的成果，不在 vault 內：33 頁教學簡報 PDF（`Downloads/ICCAD2026_electro_v22_deck_v2.pdf`，含數學推導、手繪 SVG 圖解、12 道考題）與 Zotero 資料夾「ICCAD 2026 C — electro_v22 理論基礎」（7 篇 + 一份掛在 FloorSet 項目下的閱讀指南子筆記）。簡報第一版封面把 Avg Runtime 1.91s 當成演算法屬性報告，發現七次量測散布 1.91–2.12s 之後已更正為只報確定性指標。

## [2026-08-26] Update | 防崩潰/穩健性 campaign：拿真實隱藏測資失敗紀錄反查根因，補齊生產路徑安全洞

- **Context**: 使用者問「有沒有接近 beta hidden case 的測資？」。`official guidelines/` 裡有一份 2026-08-15 真實送測隱藏測資（100 案）的完整執行紀錄（`beta_evaluation_results.json` + `eval_op_wrapper.log`），Total Score 1.5766，**10/100 不可行**，用的是舊版程式碼。
- **關鍵發現**：比對兩份檔案，**10 個不可行案例的 `max_area_drift`（診斷用的面積漂移量）全部 >0.01（0.0111–0.0507），其餘 90 案全部精確等於 0——100% 相關，一個不多一個不少**。對應 block_count（=21+test_id）：23, 25, 27, 28, 34, 37, 45, 53, 64, 76。往回查代碼發現這個根因**其實已經被查到並修好**：`electro_optimizer.py` 的 AREA GUARD（2026-08-18，面積硬約束從「上游預算假設」改成「主動檢查+等比例縮放校正」）與 `ELECTRO_MIB_PERBLOCK_AREA`（2026-08-25，AREA GUARD 觸發率訓練集配對驗證降 99.65%）都明確引用這個發現當修復動機——兩者都早於這次調查，但**都還沒有針對這 10 個真實踩過的具體尺寸做過直接驗證**。
- **本輪新增的驗證**：從訓練集在這 10 個 n 值各抓 4 案（共 40 案），新增進 `collaborate/scripts/default_stress_cases.json` 的新類別 `beta_hidden_known_risky_n`（清單累積到 192 案）。用現在的生產程式碼跑：**40 案全部可行，`max_area_drift` 全部乾淨落在門檻（0.0098）下方的 0.0091，沒有一案觸發 AREA GUARD 硬矯正**——第一次能對著真實隱藏測資的失敗案例做直接迴歸驗證，不只是訓練集配對統計。
