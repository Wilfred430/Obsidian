...
- **畫布（Canvas / Outline）** = 這座城市的**土地邊界**。⚠️ 注意：在本競賽（ICCAD 2026 FloorSet-Lite）裡，畫布大小**不是**預先給定、不能超越的固定框——它是「把所有建築物擺好之後，量出來的最小外接矩形」，量出來的面積跟 baseline 比差距去扣分，不是「蓋出界就判死」。
...
**硬規矩（Fixed-shape / Preplaced / 不重疊 / 面積誤差 ≤1%，共 4 類）違反了直接判定「不合法」**（判 Cost=10，本庫術語：**infeasible**，相對地，合法解叫 **feasible**）。**注意這裡沒有「蓋出畫布外」這一條**——官方沒有預先給定的固定畫布，畫布大小是量出來的，蓋多大只會反映在面積分數，不會判死。**軟規矩（Boundary / Grouping / MIB）違反不會判死刑，但會被扣分，而且扣分是指數成長的**（詳見 [[ICCAD_code/3_Cost_Function_and_Penalty|Cost Function 筆記]]）。
